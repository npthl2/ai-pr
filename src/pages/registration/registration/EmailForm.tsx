...
const EmailForm = ({ status, onSendEmail, isEnabled, isLoading = false }: EmailFormProps) => {
  ...
  return (
    <Box sx={{ width: '100%', p: 1 }}> {/* 최상위 컨테이너: 전체 너비 사용, 패딩 추가 */}
      <Box sx={{ 
          ...
          <TextField
            ...
            sx={{
              ...
              '& .MuiOutlinedInput-root': { // 입력 필드 루트 요소 스타일
                ...
                '&.Mui-focused fieldset': { // 포커스 상태 테두리 스타일
          ...
          <Select
            ...
            sx={{
              ...
              '& .MuiOutlinedInput-root': { // 입력 필드 루트 요소 스타일
                ...
                '&.Mui-focused fieldset': { // 포커스 상태 테두리 스타일
                  borderColor: !isCustomDomain && errors.emailDomain ? 'error.main' : 'primary.main' // 오류 상태에 따른 포커스 테두리 색상
                }
              }
            }}
            MenuProps={{
