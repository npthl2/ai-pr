...
const SatisfactionSurvey = () => {
  ...
  return (
    <SurveyCard
      disabled={surveyResponseStatus?.hasResponseYn}
      onClick={surveyResponseStatus?.hasResponseYn === 'Y' ? undefined : handleCardClick}
    >
      <Container>
        ...
        <SecondaryTypography variant='body1'>
          {surveyResponseStatus?.hasResponseYn === 'Y'
            ? '참여해주셔서 감사합니다:)'
            : '소중한 의견을 남겨주세요.'}
        </SecondaryTypography>
      </Container>
      <Typography variant='body1' sx={{ position: 'absolute', top: '108px', left: '27px' }}>
        {surveyResponseStatus?.totalResponses ?? ''}명 참여완료!
      </Typography>
      <SurveyResponseModal
        open={isOpen}
        onClose={() => {
