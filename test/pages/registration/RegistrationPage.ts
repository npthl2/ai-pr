...
class RegistrationPage {
  ...
  toggleEmailSending(enable: boolean) {
    cy.get('[data-testid="email-toggle"]').invoke('prop', 'checked').then((checked) => {
  ...
  assertEmailToggleDisabled() {
    ...
    cy.get('[data-testid="email-toggle"]').invoke('prop', 'checked').then((initialChecked) => {
      // 클릭 시도
      cy.get('[data-testid="email-toggle"]').parent().click({ force: true });
      
      // 상태가 변하지 않았는지 확인 (비활성화 상태이므로 변하지 않아야 함)
      cy.get('[data-testid="email-toggle"]').invoke('prop', 'checked').should('eq', initialChecked);
    });
  }
