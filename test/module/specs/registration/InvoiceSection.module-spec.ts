...
describe('KAN-7 신규가입 진입', () => {
  ...
  it('KAN-37-1 청구정보가 열린 경우 수령인이 포커스 되어있다.', () => {
    page.assertComponentToBeFocused(RECIPIENT_INPUT_NAME);
  });

  // 청구정보 있는 경우
  it('KAN-37-2 기존의 청구정보가 있는 경우 청구정보조회 버튼 클릭시 청구정보조회 모달이 열린다', () => {
  ...
  it('KAN-37-3 청구정보조회 모달에서 청구정보를 선택하고 선택버튼을 눌렀을때 모달이 닫히며 청구정보영역에 청구정보가 보인다', () => {
  ...
  it('KAN-37-4 청구정보가 없는 경우 청구정보조회 버튼이 비활성화 된다.', () => {
    // 재진입
    bookmarkService.successWhenGetBookmarkList();
    customerDetailService.successWhenGetCustomerContracts();
  ...
  it('KAN-37-5 발송형태를 이메일로 변경했을때 이메일 입력 영역이 보인다', () => {
  ...
  it('KAN-37-6 발송형태를 모바일로 변경했을때 이메일 입력 영역이 사라진다', () => {
  ...
  it('KAN-37-7 납부방법을 카드로 변경했을때 카드정보 입력 영역이 보인다', () => {
  ...
  it('KAN-37-8 납부방법을 지로로 변경했을때 카드정보 입력 영역이 사라진다', () => {
  ...
  it('KAN-37-9 납부방법을 은행으로 변경했을때 은행계좌정보 입력 영역이 보인다', () => {
  ...
  it('KAN-37-10 청구주소 검색버튼을 클릭했을때 주소검색 모달이 열린다', () => {
    page.clickInvoicePostalCodeSearchIcon();
    page.assertComponentToBeVisible(ADDRESS_SEARCH_MODAL_NAME);
  });

  it('KAN-37-11 정보를 입력하고 청구정보생성완료 버튼을 클릭했을때 입력한 정보가 보이고 청구정보조회 버튼이 disalble 상태가 되며 청구정보생성완료 버튼이 사라진다.', () => {
