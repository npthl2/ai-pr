...
describe('KAN-22 요금제/부가서비스 변경', () => {
  ...
  describe('KAN-22-3 부가서비스 목록', () => {
    ...
    it('KAN-22-3-7 가입 불가 부가서비스에 마우스를 hover 했을 때 툴팁이 보여야 한다', () => {
      ...
      // 툴팁이 표시되는지 확인
      page.assertTooltipTextEquals('요금제 제한으로 인해 가입이 불가능합니다.');
    });
    ...
    it('KAN-22-3-12 선택된 부가서비스 목록도 정렬 가능해야 한다', () => {
      ...
      page.clickSelectedAdditionalServiceNameSort();
      cy.wait(500);
      page.clickSelectedAdditionalServiceNameSort();
      page.assertSelectedTopAdditionalServiceIs('LTE 3 Giga 충전 부가서비스');

      cy.wait(500);

      // 선택된 부가서비스 목록의 요금 정렬 클릭
      page.clickSelectedAdditionalServicePriceSort();
      cy.wait(500);
      page.clickSelectedAdditionalServicePriceSort();
      page.assertSelectedTopAdditionalServiceIs('LTE 3 Giga 충전 부가서비스');
    });

    it('KAN-22-3-13 삭제 버튼 클릭 시 부가서비스 목록으로 이동해야 한다', () => {
      service.noAgeRestrictionWhenCheckServiceAgeRestriction();
      service.successWhenGetAdditionalServices();
      ...
      serviceSearchPage.getServiceInfoChangeButton().click();
      // 선택된 부가서비스의 삭제 버튼 클릭
      page.clickRemoveSelectedServiceButton('5G 3 Giga 충전 부가서비스');

      page.assertAdditionalServiceInList('5G 3 Giga 충전 부가서비스');
    });

    it('KAN-22-3-14 선택된 부가서비스 목록이 변경되면 합계 금액이 변경되어야 한다', () => {
      service.noAgeRestrictionWhenCheckServiceAgeRestriction();
      service.successWhenGetAdditionalServices();
      ...
      // 합계 금액이 변경되었는지 확인
      page.assertTotalAmountIs('24,000원');
    });
  ...
  describe('KAN-22-4 저장 / 초기화', () => {
    ...
    it('KAN-22-4-8 선택된 부가서비스가 현재 부가서비스로 변경되어야 한다', () => {
      ...
      cy.wait(500);
      // 부가서비스가 초기화되었는지 확인
      page.assertSelectedAdditionalServiceIs('5G 3 Giga 충전 부가서비스');
    });
  });
});
